...
public class BudgetPlanGUI extends JFrame {
	...
	protected void initWindow() {
		...
		JFreeChart balken = ChartFactory.createBarChart("Einnahmen",
				"Kategorie", "Betrag", dataset, PlotOrientation.VERTICAL, true,
				true, false);
		...
		JFreeChart balken2 = ChartFactory.createBarChart("Ausgaben",
				"Kategorie", "Betrag", dataset2, PlotOrientation.VERTICAL,
				true, true, false);
	...
	public Object[][] setupTable(boolean einkommen, Date von) {
		...
		int i = 0;
		for (Posten p : budget.gesamt) {
			if (((einkommen && p.getBetrag() >= 0) || (!einkommen && p
					.getBetrag() < 0))
					&& (p.getDatum().after(von) || p.getDatum().equals(von))) {
				data[i][0] = new SimpleDateFormat("dd/MM/yyyy").format(p
						.getDatum());
				data[i][1] = p.getBezeichnung();
				data[i][2] = String.format("%.2f", p.getBetrag());
		...
		ActionListener aListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (e.getSource() == buttonAll) {
					...
					dtmIn = new DefaultTableModel(setupTable(true, getYear()),
							line);
					dtmOut = new DefaultTableModel(
							setupTable(false, getYear()), line);
					tableIn.setModel(dtmIn);
					tableOut.setModel(dtmOut);
					dtmIn.fireTableDataChanged();
					dtmOut.fireTableDataChanged();
				} else if (e.getSource() == buttonMonth) {
					dtmIn = new DefaultTableModel(setupTable(true, getMonth()),
							line);
					dtmOut = new DefaultTableModel(
							setupTable(false, getMonth()), line);
					tableIn.setModel(dtmIn);
					tableOut.setModel(dtmOut);
					dtmIn.fireTableDataChanged();
					dtmOut.fireTableDataChanged();
